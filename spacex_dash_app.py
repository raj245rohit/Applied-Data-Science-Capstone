...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        for i, row in filtered_df.iterrows():
            if row['class'] == 0:
                SuccessRate.append('Failed')
            else:
                SuccessRate.append('Success')
        filtered_df['Success Rate'] = SuccessRate
        grouped_df = filtered_df.groupby(['Success Rate'], as_index=False)['class'].count()
...
def get_scatter_chart(entered_site, payload):
    if entered_site == 'ALL':
        ...
        y='class',
        color="Booster Version Category", 
        title='Corelation between Payload and Success for selected Site')
        return fig
